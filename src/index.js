{
  ...
  function createCustomLinkFields() {
    ...
    label.appendChild(
      document.createTextNode(`Custom Link #${linksIndex - 4}`)
    );
  ...
  const additionalLinkButton = document.getElementById("additionalLink");
  ...
  formData["photo"].addEventListener("input", (e) => {
    let photoData = e.target.files[0];
    if (photoData) {
  ...
  function UpdatePreview(replaceGroup = null) {
    let name = formData["name"].value;
    let mainUrl = formData["url"].value;
    ...
    // Links
    {
      ...
      for (var i = 0; i <= linksIndex; i++) {
        ...
        if (linkUrl !== "") {
          let userLink = createLinkElement(
            linkUrl,
            linkIcon,
            linkName,
            "_blank"
  ...
  for (var i = 0; i <= linksIndex; i++) {
    ...
    document
      ...
      .addEventListener("input", () => UpdatePreview("links"));
    document
    ...
    document
      ...
      .addEventListener("blur", () => UpdatePreview("links"));
  }

  // Add Listener for all forms inputs on file Load
  formData["name"].addEventListener("input", () => UpdatePreview("top"));
  formData["url"].addEventListener("input", () => UpdatePreview("top"));
  formData["description"].addEventListener("input", () => UpdatePreview("top"));
  formData["email"].addEventListener("input", () => UpdatePreview("links"));
  theme.addEventListener("input", () => UpdatePreview("theme"));
